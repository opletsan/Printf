...
static void			src_spec_c_s(t_prnt *s, char spec)
{
	...
	{
		if (!(str = (wchar_t*)va_arg(s->ap, uintmax_t)))
			use_flag_str(s, NULL, 0);
		else
			use_flag_unistr(s, str, 0);
	}
	else if (spec == 's')
		use_flag_str(s, (unsigned char*)va_arg(s->ap, uintmax_t), 0);
}
